...
\begin{figure}[H]%
    ...
    {{\includegraphics[width=0.55\textwidth]{figures/dConventionalThreeLevelBC/PWM1.eps} }}%
    \qquad
    \subfloat{{\includegraphics[width=0.55\textwidth]{figures/dConventionalThreeLevelBC/PWM2.eps} }}%  
    \caption{Switching pattern for 60\%}%
     \label{fig:PWM12}% 
\end{figure}

\todo[color=c04d]{Figure in appendix on top of each other}
\todo[color=c04d]{No need anymore, ho-ho-ho} 
...
To find an expression for the output-input gain of the topology, we refer back to the IVSB (Sec. \ref{sec:conversionRatio}) and formulate the expression that is valid for both switches since Eq. \ref{eq:CTLBC_KVL_ONOFF} and Eq. \ref{eq:CTLBC_KVL_OFFON} are identical:
