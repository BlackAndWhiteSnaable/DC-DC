\section{Nx Multilevel BC}\label{ch:MBC}

\todo[color=c04y,inline]{testing the colors}

The Nx Multilevel Boost Converter (Figure~\ref{fig:MBC_3XFULL}) is based on one switching device, 2N - 1 diodes and 2N - 1 capacitors, where N indicates the level number of the converter and can be extended by adding capacitors and diodes while the main circuit is not required to be changed. 
...
\begin{equation}
	\Rightarrow \frac{V_o}{V_{in}} = \frac{N}{(1-D)}
	\label{eq:MBC_INVSB3}
\end{equation}
